...
const WeightProductsScreen = () => {
  ...
  // Filter products based on search and category - ONLY WEIGHABLE PRODUCTS
  useEffect(() => {
    ...
    // Filter by weighable products only (not 'unit' type)
    filtered = filtered.filter(product => 
      product.price_type && product.price_type !== 'unit'
    );
  ...
  const updateSelectedWeight = (productId, weight) => {
    ...
    setSelectedProducts(selectedProducts.map(item => 
      item.id === productId ? { ...item, weight: numWeight } : item
    ));
  };
  ...
  // Get unique categories from weighable products only
  const getCategories = () => {
    const weighableProducts = products.filter(p => p.price_type && p.price_type !== 'unit');
    const categories = [...new Set(weighableProducts.map(p => p.category).filter(Boolean))];
    ...
    >
      ...
      {selectedProducts.length > 0 && (
        <View style={styles.selectionCartHeader}>
          <View style={styles.selectionCartInfo}>
            <Icon name="scale" size={20} color="#f97316" />
            <Text style={styles.selectionCartText}>
              {selectedProducts.length} weighable product{selectedProducts.length !== 1 ? 's' : ''} selected • {formatCurrency(getSelectedTotal())}
            </Text>
          </View>
          <TouchableOpacity 
      ...
      <View style={styles.ultimateHeader}>
        ...
        <View style={styles.commandCenterBadge}>
          <Icon name="scale" size={20} color="#f97316" />
          <Text style={styles.commandCenterBadgeText}>WEIGHABLE PRODUCTS</Text>
        </View>
        
        <Text style={styles.ultimateHeaderTitle}>⚖️ Weight Products</Text>
        
        <View style={styles.ultimateHeaderSubtitleContainer}>
          <Icon name="store" size={16} color="#f97316" />
          <Text style={styles.ultimateHeaderSubtitle}>Browse and Add Weighable Items</Text>
          <Icon name="shopping-cart" size={16} color="#3b82f6" />
        </View>
        
        <View style={styles.realtimeStatus}>
          <View style={styles.statusDot} />
          <Text style={styles.statusText}>{filteredProducts.length} Weighable Products</Text>
          <Icon name="wifi" size={14} color="#f97316" />
        </View>
        
        <View style={styles.performanceSummary}>
          <Text style={styles.performanceSummaryText}>
            ⚖️ {filteredProducts.length} Products Displayed • {getCategories().length - 1} Categories
          </Text>
        </View>
      </View>
      ...
      <View style={styles.searchFilterSection}>
        ...
        <View style={styles.searchContainer}>
          <Text style={styles.searchLabel}>🔍 SEARCH WEIGHABLE PRODUCTS:</Text>
          <View style={styles.searchInputContainer}>
            <TextInput
              style={styles.searchInput}
              value={searchQuery}
              onChangeText={setSearchQuery}
              placeholder="Search weighable products by name, code, or barcode..."
      ...
      <View style={styles.productsContainer}>
        {productsLoading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Loading weighable products...</Text>
          </View>
        ) : filteredProducts.length === 0 ? (
          <View style={styles.noProductsContainer}>
            <Icon name="scale" size={64} color="#6b7280" />
            <Text style={styles.noProductsText}>
              {searchQuery || selectedCategory !== 'all' 
                ? 'No weighable products match your search criteria'
                : 'No weighable products available'
              }
            </Text>
            <TouchableOpacity 
              ...
              >
                ...
                <View style={styles.productCardBody}>
                  <View style={styles.productInfo}>
                    ...
                    <View style={styles.stockIndicator}>
                      ...
                      <Text style={[
                        ...
                        📦 {product.stock_quantity || 0} {product.price_type}
                  ...
                  <View style={styles.productPriceSection}>
                    <Text style={styles.priceText}>{formatCurrency(product.price)}/{product.price_type}</Text>
                    <TouchableOpacity
                      style={[
                        styles.addButton,
                    ...
                    >
                      <Text style={[
                        ...
                        {selectedProducts.find(item => item.id === product.id) ? '✓ SELECTED' : '⚖️ SELECT'}
                      </Text>
                    </TouchableOpacity>
                    
                    {/* Weight controls for selected products */}
                    {selectedProducts.find(item => item.id === product.id) && (
                      <View style={styles.weightControls}>
                        <Text style={styles.weightLabel}>Weight ({product.price_type}):</Text>
                        <TextInput
                          style={styles.weightInput}
                          value={selectedProducts.find(item => item.id === product.id)?.weight?.toString() || ''}
                          onChangeText={(text) => updateSelectedWeight(product.id, text)}
                          placeholder={`0.0`}
                          keyboardType="numeric"
                        />
                        <Text style={styles.unitLabel}>{product.price_type}</Text>
                      </View>
                    )}
                  </View>
                </View>
                ...
                <View style={styles.productCardFooter}>
                  <Text style={styles.cardVintageBorder}>⚖️ Premium Weighable Products</Text>
...
const styles = StyleSheet.create({
  ...
  headerBackgroundOverlay: {
    ...
    right: 0,
    bottom: 0,
    backgroundColor: 'linear-gradient(135deg, rgba(249, 115, 22, 0.1) 0%, rgba(245, 158, 11, 0.1) 100%)',
  },
  commandCenterBadge: {
    flexDirection: 'row',
    alignItems: 'center',
...
export default WeightProductsScreen;