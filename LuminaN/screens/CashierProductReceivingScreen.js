...
const CashierProductReceivingScreen = () => {
  ...
  const loadInitialData = async () => {
    try {
      // Load products only (suppliers and history not available yet)
  ...
  const generateInvoiceNumber = () => {
    ...
    const invoice = `INV-${new Date().getFullYear()}${String(new Date().getMonth() + 1).padStart(2, '0')}${new Date().getDate()}-${timestamp}`;
  ...
  const saveReceivingLocally = async () => {
    try {
      console.log('💾 Saving receiving to storage...');
      
      // Generate unique receiving ID
      const uniqueReceivingId = `REC-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`;
      console.log('📋 Generated Receiving ID:', uniqueReceivingId);
      
      // Get existing receiving records using the dedicated method
      let existingReceiving = await shopStorage.getCashierReceivingRecords();
      
      // Create receiving data - should be pending review, not received
      // Get cashier name from credentials
      const cashierName = shopCredentials?.name || shopCredentials?.cashier_name || shopCredentials?.username || 'Unknown Cashier';
      console.log('💰 Debug cashier name extraction:', {
        cashierName,
        shopCredentialsName: shopCredentials?.name,
        shopCredentialsCashierName: shopCredentials?.cashier_name,
        shopCredentialsUsername: shopCredentials?.username,
        shopCredentialsKeys: Object.keys(shopCredentials || {})
      });
      
      const receivingData = {
        id: uniqueReceivingId,
        ...
        status: 'pending_review',  // Changed from 'received' to 'pending_review'
        createdAt: new Date().toISOString(),
        createdBy: 'cashier',
        cashierName: cashierName  // Store actual cashier name
      };
      
      console.log('📝 Receiving data:', receivingData);
      console.log('💰 Debug receiving data cashier name:', {
        cashierName: receivingData.cashierName,
        createdBy: receivingData.createdBy,
        fullReceivingData: receivingData
      });
      
      // Add to receiving records and save using the dedicated method
      const saveResult = await shopStorage.saveCashierReceivingRecord(receivingData);
      
      console.log('✅ Receiving saved! Total records:', existingReceiving.length);
      return true;
  ...
  const handleReceiveProducts = async () => {
    ...
    // For web platform, use a custom confirmation instead of Alert
    if (Platform.OS === 'web') {
      console.log('🌐 Web platform detected - using custom confirmation');
      const confirmed = window.confirm(
        `You are about to receive ${totals.totalItems} items worth ${totals.totalValue.toFixed(2)} from ${supplierName}.\n\nThis will be recorded as received products.`
      );
      
      if (confirmed) {
        await processReceivingConfirmation();
      } else {
        console.log('❌ User cancelled receiving');
      }
    } else {
      // Show confirmation dialog for mobile
      console.log('📱 Mobile platform - showing Alert dialog');
      Alert.alert(
        ...
        [
          {
            text: 'Cancel',
            style: 'cancel',
            onPress: () => console.log('❌ User cancelled receiving')
          },
          {
            text: 'Confirm Receiving',
            onPress: processReceivingConfirmation
          }
        ]
      );
    }
  ...
  const processReceivingConfirmation = async () => {
    console.log('✅ User confirmed receiving - starting process');
    setSaving(true);
  ...
  const handleSuccessModalOK = () => {
    ...
    // Reset form after successful receiving
    resetForm();
  };
  ...
  const renderHeader = () => (
    <View style={styles.header}>
      ...
      <Text style={styles.headerTitle}>📦 Cashier Product Receiving</Text>
  ...
  const renderReceivingDetails = () => (
    <View style={styles.formCard}>
      ...
      {/* Receive Products Button (No confirmation/approval) */}
      <TouchableOpacity
        style={[
          styles.submitButton,
          (receivingItems.length === 0 || !supplierName.trim()) && styles.submitButtonDisabled
        ]}
        onPress={handleReceiveProducts}
        disabled={receivingItems.length === 0 || !supplierName.trim() || saving}
      >
        <Text style={styles.submitButtonText}>
          {saving ? '⏳ Processing...' : '📦 Receive Products'}
        </Text>
      </TouchableOpacity>
    </View>
  );
    ...
    >
      <View style={styles.successModalContainer}>
        <View style={styles.successModalContent}>
          ...
          {/* Success Title */}
          <Text style={styles.successTitle}>Products Received Successfully!</Text>
          
          {/* Success Message */}
          <Text style={styles.successMessage}>
            Your product receiving has been recorded and saved.
          </Text>
          ...
          <View style={styles.successDetails}>
            ...
            <Text style={styles.successDetail}>🏢 Supplier: {supplierName}</Text>
...
export default CashierProductReceivingScreen;