    ...
    try:
        ...
        if today_sales.exists() and drawers.exists():
            for sale in today_sales:
                ...
                if matching_drawer:
                    print(f"   SUCCESS: Sale #{sale.id} by {sale.cashier.name} -> Drawer for {matching_drawer.cashier.name}")
                else:
                    print(f"   ERROR: Sale #{sale.id} by {sale.cashier.name} -> NO MATCHING DRAWER!")
...
if __name__ == "__main__":
    ...
    if has_discrepancy:
        print(f"\nDISCREPANCY DETECTED - Running fix...")
