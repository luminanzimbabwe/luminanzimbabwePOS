#!/usr/bin/env python3
"""
Test script to validate drawer sales attribution fix
This script tests that sales are correctly attributed to the right cashier's drawer
"""
...
def test_drawer_attribution():
    """Test that sales are correctly attributed to cashier drawers"""
    ...
    try:
        ...
        if created1:
            print(f"✅ Created test cashier 1: {cashier1.name} (ID: {cashier1.id})")
        else:
            print(f"✅ Using existing cashier 1: {cashier1.name} (ID: {cashier1.id})")
            
        if created2:
            print(f"✅ Created test cashier 2: {cashier2.name} (ID: {cashier2.id})")
        else:
            print(f"✅ Using existing cashier 2: {cashier2.name} (ID: {cashier2.id})")
        ...
        if created3:
            print(f"✅ Created test product: {product1.name}")
        else:
        ...
        if created4:
            print(f"✅ Created drawer for cashier 1")
        else:
            print(f"✅ Using existing drawer for cashier 1")
            
        if created5:
            print(f"✅ Created drawer for cashier 2")
        else:
            print(f"✅ Using existing drawer for cashier 2")
        ...
        print(f"✅ Created sale #{sale1.id} for {cashier1.name}")
        ...
        print(f"✅ Created sale #{sale2.id} for {cashier2.name}")
        ...
        if drawer1.session_total_sales == expected_cashier1_sales:
            print(f"✅ Cashier 1 drawer correctly shows ${expected_cashier1_sales} in sales")
        else:
            print(f"❌ Cashier 1 drawer shows ${drawer1.session_total_sales}, expected ${expected_cashier1_sales}")
        
        if drawer2.session_total_sales == expected_cashier2_sales:
            print(f"✅ Cashier 2 drawer correctly shows ${expected_cashier2_sales} in sales")
        else:
            print(f"❌ Cashier 2 drawer shows ${drawer2.session_total_sales}, expected ${expected_cashier2_sales}")
        ...
        if drawer1.current_cash == expected_cashier1_cash:
            print(f"✅ Cashier 1 drawer correctly shows ${expected_cashier1_cash} in cash")
        else:
            print(f"❌ Cashier 1 drawer shows ${drawer1.current_cash}, expected ${expected_cashier1_cash}")
        
        if drawer2.current_cash == expected_cashier2_cash:
            print(f"✅ Cashier 2 drawer correctly shows ${expected_cashier2_cash} in cash")
        else:
            print(f"❌ Cashier 2 drawer shows ${drawer2.current_cash}, expected ${expected_cashier2_cash}")
        ...
        if (drawer1.session_total_sales == expected_cashier1_sales and 
            ...
            print("🎉 ALL TESTS PASSED! Drawer sales attribution is working correctly.")
            return True
        else:
            print("❌ TESTS FAILED! There are issues with drawer sales attribution.")
            return False
